---
title: Development
permalink: "/resources/development/"
layout: page
---

### Python

* [Managing environments with conda](https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html)
* [What's new in Python](https://docs.python.org/3/whatsnew/index.html)
* [PEP 8 - Style Guide for Python Code](https://www.python.org/dev/peps/pep-0008/)
* [NumPy docstrings](https://numpydoc.readthedocs.io/en/latest/format.html)

### Python Patterns

* [The Zen of Python](https://www.python.org/dev/peps/pep-0020/)
* [The Hitchhiker's Guide to Python](https://docs.python-guide.org/)
* [Python Sensibilities](https://docs.python-guide.org/)
* [Design Patterns in Python](https://refactoring.guru/design-patterns/python)
* [Python Patterns](https://python-patterns.guide/)
* [Python Clean Code](https://github.com/zedr/clean-code-python/blob/master/README.md)
* [What's __init__ for me?](https://towardsdatascience.com/whats-init-for-me-d70a312da583)

### Data Science in Practice

* [Python Data Science Handbook](https://jakevdp.github.io/PythonDataScienceHandbook/)
* [PyData Youtube Channel](https://www.youtube.com/channel/UCOjD18EJYcsBog4IozkF_7w)

### Deploying Machine Learning

* [Deploying Python Machine Learning Models in Production](https://www.youtube.com/watch?v=6TI-gQhsf40)
* [How to Deploy a Tensorflow Model to Production](https://www.youtube.com/watch?v=T_afaArR0E8)
* [Deploying and Managing Machine Learning Models](https://www.youtube.com/watch?v=q-VPALG6ogY)
* [Deploying Python models to production](https://www.youtube.com/watch?v=f3I0izerPvc)
* [Hidden Technical Debt in Machine Learning Systems](http://papers.nips.cc/paper/5656-hidden-technical-debt-in-machine-learning-systems.pdf)

### Jupyter Notebooks

* [Jupyter notebook tutorial](https://www.dataquest.io/blog/jupyter-notebook-tutorial/)
* [A gallery of interesting Jupyter notebooks](https://github.com/jupyter/jupyter/wiki/A-gallery-of-interesting-Jupyter-Notebooks)

### NumPy

* [101 NumPy exercises](https://www.machinelearningplus.com/python/101-numpy-exercises-python/)
* [A hitchhiker's guide to Python NumPy arrays](https://towardsdatascience.com/a-hitchhiker-guide-to-python-numpy-arrays-9358de570121)
* [Broadcasting](https://docs.scipy.org/doc/numpy/user/basics.broadcasting.html)
* [Look Ma, No For-Loops: Array Programming with NumPy](https://realpython.com/numpy-array-programming/)
* [Losing Your Loops: Fast Numerical Computing with NumPy (Jake VanderPlas)](https://www.youtube.com/watch?v=EEUXKG97YRw)

### scipy

* [SciPy Lectures](https://scipy-lectures.org/)

### pandas

* [10 minutes to pandas](https://pandas.pydata.org/pandas-docs/stable/getting_started/10min.html)
* [Effective pandas](https://github.com/TomAugspurger/effective-pandas)
* [Maintaining performance](http://tomaugspurger.github.io/maintaing-performance.html)

### scikit-learn

* [A deep dive into sklearn pipelines](https://www.kaggle.com/baghern/a-deep-dive-into-sklearn-pipelines)

### Plotting

* [Python Graph Gallery](https://python-graph-gallery.com/)
* [Matplotlib Gallery](https://matplotlib.org/stable/gallery/index.html)
* [Seaborn Gallery](https://seaborn.pydata.org/examples/index.html)
* [Plotly Examples](https://plotly.com/python/)

### Flask

* [Python machine learning Flask example](https://www.toptal.com/python/python-machine-learning-flask-example)
* [Building a simple Keras deep learning REST API](https://blog.keras.io/building-a-simple-keras-deep-learning-rest-api.html)
* [The Flask mega tutorial](https://blog.miguelgrinberg.com/post/the-flask-mega-tutorial-part-i-hello-world)

### Version Control

* [Resources to learn git](https://try.github.io/)
* [My favourite git commit](https://fatbusinessman.com/2019/my-favourite-git-commit)
* [Telling stories through your commits](https://blog.mocoso.co.uk/talks/2015/01/12/telling-stories-through-your-commits/)
* [How to write a git commit message](https://chris.beams.io/posts/git-commit/)
* [A talk about revision histories](https://tekin.co.uk/2019/02/a-talk-about-revision-histories)
* [A successful git branching model](https://nvie.com/posts/a-successful-git-branching-model/)
* [Semantic Versioning](https://semver.org/)

### SQL

* [SQL Style Guide](https://www.sqlstyle.guide/)

### Docker

* [Docker Curriculum](https://docker-curriculum.com/#introduction)

### Kubernetes

* [Kubernetes Explainer (11 mins)](https://www.youtube.com/watch?v=aSrqRSk43lY)
* [Learn Kubernetes in under 3 hours](https://medium.freecodecamp.org/learn-kubernetes-in-under-3-hours-a-detailed-guide-to-orchestrating-containers-114ff420e882)
* [Kubernetes Standardised Glossary](https://kubernetes.io/docs/reference/glossary/?fundamental=true)

### Airflow

* [Introduction to Apache Airflow](https://www.applydatascience.com/airflow/airflow-tutorial-introduction/)
* [Airflow tutorial](https://github.com/hgrif/airflow-tutorial)
* [Getting started with Apache Airflow](https://towardsdatascience.com/getting-started-with-apache-airflow-df1aa77d7b1b)
* [Airflow Pitfalls](https://cwiki.apache.org/confluence/display/AIRFLOW/Common+Pitfalls)
* [ETL with Airflow - Principles](https://gtoonstra.github.io/etl-with-airflow/principles.html)

### Data Competitions
* [Kaggle](www.kaggle.com)

### Datasets

* [100+ Interesting Data Sets for Statistics](http://rs.io/100-interesting-data-sets-for-statistics/).
* [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/index.html)
